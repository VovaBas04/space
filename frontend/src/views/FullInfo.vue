...
export default {
  name: "FullInfo",
  components: {StatisticSend, Logo, SupportInfo, Description},
  methods: {
    descriptionInfo: async function () {
      let data = (await (await fetch(process.env.VUE_APP_BACK_URL + `/api/v1/space-port/${this.$route.params.id}/description`)).json())
...
<template>
  <div class="grid grid-cols-3 text-white ">
    <Logo :path-flag=path />
    <h1 class="col-span-2 mb-2  text-6xl  font-bold  tracking-tight  dark:text-white">{{this.description.title}}</h1>
    <Description class="col-span-3"/>
    <SupportInfo class="col-span-2" title="Местоположение,занимаемая площадь" :description=description.aboutPlace >
      Занимаемая площадь:{{this.description.area}}
    </SupportInfo>
    <SupportInfo class="col-span-2" title="Назначение" :description=description.target >
    </SupportInfo>
  </div>
</template>
